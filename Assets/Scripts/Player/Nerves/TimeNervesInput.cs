...
namespace Player
{
    ...
    /// value that recovery cannot push below. Floor grows over time, scaled by
    /// GameManager.Difficulty. Does not contribute through the normal delta
    /// channel — recovery would otherwise overwhelm the slow rate.
    /// </summary>
    public class TimeNervesInput : NervesInput
    {
        ...
        [Tooltip("Base floor growth per second at difficulty 1, just past the grace period.")]
        ...
        private void Update()
        {
            ...
            float ratePerSecond = baseRatePerSecond * timeMult * diffMult;
